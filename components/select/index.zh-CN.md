...
| labelInValue | 是否把每个选项的 label 包装到 value 中，会把 Select 的 value 类型从 `string` 变为 `{key: string, label: vNodes, originLabel: any}` 的格式, originLabel（3.1） 保持原始类型，如果通过 pf-select-option children 构造的节点，该值是是个函数（即 pf-select-option 的默认插槽） | boolean | false |  |
